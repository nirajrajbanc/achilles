...
if(validators.url(url)):
	...
	if(config['forms']):
		for form in forms:
			if((form.get('action').find('https') < 0) and (urlparse(url).scheme != 'https')):
				from_is_secure = False
				report += 'Form Issue: Insecure form action ' + form.get('action') +' found in docment\n'
	...
	print('Invalid URL. Please use full URL including scheme')

if(report == ''):
	report += "Nice job! Your HTML document is secured!"
else:
